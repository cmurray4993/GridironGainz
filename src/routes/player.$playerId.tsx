...
export const Route = createFileRoute("/player/$playerId")({
  ...
  head: () => ({ meta: [{ title: "Player — Gridiron Gainz" }] }),
});
function PlayerPage() {
  return (
...
function PlayerManager() {
  ...
  const starter = Object.values(state.lineup).includes(playerId);
  ...
  const [startingBid, setStartingBid] = useState(String(floors.startingBid));
  const [coinBuy, setCoinBuy] = useState(String(floors.coinBuyNow));
  const [solBuy, setSolBuy] = useState("0.05");
  const [duration, setDuration] = useState("24");
  const [busy, setBusy] = useState(false);
  const validation = useMemo(() => {
    if (!auctionOn && !coinOn && !solOn) return "Enable at least one sale option";
    if (auctionOn && Number(startingBid) < floors.startingBid)
      return `Minimum starting bid is ${floors.startingBid.toLocaleString()} coins`;
    if (coinOn && Number(coinBuy) < floors.coinBuyNow)
      return `Minimum coin Buy Now is ${floors.coinBuyNow.toLocaleString()} coins`;
    if (auctionOn && coinOn && Number(coinBuy) <= Number(startingBid))
      return "Coin Buy Now must be greater than the starting bid";
    if (
      solOn &&
      (Number(solBuy) < MIN_SOL_LAMPORTS / LAMPORTS_PER_SOL ||
        Number(solBuy) > MAX_SOL_LAMPORTS / LAMPORTS_PER_SOL)
    )
      return "SOL Buy Now must be between 0.01 and 100 SOL";
  ...
  if (!player)
    return (
      <div className="py-20 text-center">
        <h1 className="font-display text-3xl">Player not found</h1>
        <button
  ...
  async function quickSell() {
    ...
    setBusy(true);
    try {
      const server = await quickSellMarketCard(selectedPlayer.id);
      if (!server.handled) throw new Error("Server did not complete the quick sell");
      if (server.balance != null) setMarketCoinBalance(server.balance);
      await refreshAuthoritativeState();
      toast.success(
        `${selectedPlayer.name} quick-sold for ${server.price ?? sellPrice(selectedPlayer)} coins`,
      );
      await navigate({ to: "/roster" });
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "Quick sell failed");
    } finally {
      setBusy(false);
    }
  }
  async function list() {
    if (starter || validation) return;
    setBusy(true);
    try {
      await createMarketListing(selectedPlayer, {
        startingPrice: auctionOn ? Math.floor(Number(startingBid)) : undefined,
        buyNowPrice: coinOn ? Math.floor(Number(coinBuy)) : undefined,
        solLamports: solOn ? Math.round(Number(solBuy) * LAMPORTS_PER_SOL) : undefined,
        sellerWallet: solOn ? publicKey?.toBase58() : undefined,
        durationHours: Number(duration),
      });
      await refreshAuthoritativeState();
      toast.success(`${selectedPlayer.name} is now in the Auction House`);
      await navigate({ to: "/roster" });
    } catch (e) {
        ...
        <main className="mx-auto grid max-w-7xl grid-cols-[250px_minmax(320px,1fr)_330px] items-start gap-6 px-7 py-9">
          ...
          <section>
            ...
            <div className="space-y-2">
              ...
              <Stat label="Popularity" value={player.popularity} />
              <Stat label="Fan Value" value={player.fanValue} />
              <Stat label={player.signature.label} value={player.signature.value} />
            </div>
            <button
              disabled={starter || busy}
              onClick={() => void quickSell()}
              className="mt-5 w-full rounded-lg border border-destructive/50 bg-destructive/10 py-2.5 font-semibold text-destructive disabled:opacity-40"
            >
              {starter
                ? "Remove from lineup to quick sell"
          ...
          <aside className="rounded-2xl border border-white/10 bg-black/25 p-4">
            ...
            {starter ? (
              <div className="rounded-lg border border-primary/30 bg-primary/10 p-4 text-sm">
                Remove this player from the starting lineup before listing.
              </div>
            ) : (
              <>
                <Option checked={auctionOn} onChange={setAuctionOn} label="Coin starting bid">
                  <Price
                    disabled={!auctionOn}
                    value={startingBid}
                    onChange={setStartingBid}
                    suffix={`min ${floors.startingBid.toLocaleString()}`}
                  />
                </Option>
                <Option checked={coinOn} onChange={setCoinOn} label="Coin Buy Now">
                  <Price
                    disabled={!coinOn}
                    value={coinBuy}
                    onChange={setCoinBuy}
                    suffix={`min ${floors.coinBuyNow.toLocaleString()}`}
                  />
                </Option>
                <Option checked={solOn} onChange={setSolOn} label="Devnet SOL Buy Now">
                  <Price
                    disabled={!solOn}
                    value={solBuy}
                    onChange={setSolBuy}
                    ...
                    <div className="mt-2 grid gap-1">
                      {wallets.slice(0, 2).map((w) => (
                        <button
                          ...
                          className="rounded-lg border border-violet-500/40 px-2 py-2 text-xs"
                        >
                ...
                {validation && <div className="mt-3 text-xs text-amber-300">{validation}</div>}
                <button
                  disabled={busy || Boolean(validation)}
                  onClick={() => void list()}
                  className="mt-4 w-full rounded-lg bg-[image:var(--gradient-gold)] py-2.5 font-semibold text-primary-foreground disabled:opacity-40"
                >
                  {busy ? "Working…" : "List on Auction House"}
                </button>
              </>
            )}
          </aside>
  ...
  return (
    <div>
      <input
        ...
        className="w-full rounded-md border border-border bg-background px-3 py-2 disabled:opacity-40"
      />
      <div className="mt-1 text-[10px] text-muted-foreground">{suffix}</div>
    </div>
  );
}
